...
func TestDBUnmarshalOrderBook(t *testing.T) {

	obRes, err := models.UnmarshalDBOrderBookRow(idealOBRow)
	if err != nil {
		panic(err)
	}

	obRes.Time = time.Now()
	idealOBRes.Time = obRes.Time
	tsStr := fmt.Sprintf(time.Now().Format("2006:01:02 15:04"))
	obRes.Timestamp = tsStr
	idealOBRes.Timestamp = tsStr

	fmt.Println("obRes: ", obRes)
	fmt.Println("idealOBRes: ", idealOBRes)
